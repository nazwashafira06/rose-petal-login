    ...
    <main className="flex min-h-screen items-center justify-center px-4 py-12">
      <div className="w-full max-w-md animate-fade-up">
        ...
        <div className="glass-card border-rose-gold shadow-romantic rounded-2xl border p-8 sm:p-10">
          <h2 className="font-display text-2xl text-maroon">Create your account</h2>
          <p className="mt-1 text-sm text-muted-foreground">A few details to get you started</p>

          <form onSubmit={onSubmit} className="mt-6 space-y-4" noValidate>
            <Field icon={<User className="h-4 w-4" style={{ color: "#7B1D2A" }} />} label="Full name" type="text" value={fullName} onChange={setFullName} placeholder="Your name" error={errors.fullName} autoComplete="name" />
            <Field icon={<Mail className="h-4 w-4" style={{ color: "#7B1D2A" }} />} label="Email" type="email" value={email} onChange={setEmail} placeholder="you@rose.com" error={errors.email} autoComplete="email" />
            <Field
              ...
              placeholder="At least 6 characters" error={errors.password} autoComplete="new-password"
              trailing={
                <button type="button" onClick={() => setShowPw((s) => !s)} className="text-muted-foreground hover:text-maroon transition-colors" aria-label={showPw ? "Hide" : "Show"}>
                  {showPw ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                </button>
              }
            />
            <Field icon={<Lock className="h-4 w-4" style={{ color: "#7B1D2A" }} />} label="Confirm password" type={showPw ? "text" : "password"} value={confirm} onChange={setConfirm} placeholder="Repeat password" error={errors.confirm} autoComplete="new-password" />
            ...
            <p className="text-center text-sm text-muted-foreground">
              ...
              <Link to="/login" className="font-medium text-maroon hover:underline">Sign in</Link>
            </p>
          </form>
        </div>
      </div>
